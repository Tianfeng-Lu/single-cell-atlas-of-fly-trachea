...
# ctrl_filtered <- RenameIdents(ctrl_filtered, "PC_4" = "DB") #PC_4 分到DB去了
# Idents(ctrl_filtered) -> ctrl_filtered$sub.cluster
...
# ggsave(filename = "./ctrl_only/PC_subumap2.png", device = png, height = 4, width = 5, plot = PCsubumap) #对svg调整坐标轴
...
trans_prob <- matrix(nrow = cell_num, ncol = cell_num, dimnames = list(rownames(shift_vector), rownames(shift_vector))) # transition probability 是一个 #cell × #cell 的矩阵 每一行根据shift vector计算的到其他细胞转移概率
for(i in seq(cell_num)){ # 按行遍历所有细胞
  d_i <- shift_vector[i]
  r_ij <- t(ctrl_PC_expr) - t(ctrl_PC_expr)[i]
  dist_ij <- sqrt(rowSums((r_ij - d_i)^2))
  prob_ij <- exp(-minmax_scale(dist_ij)*2) # 未归一的概率
...
trans_prob2 <- trans_prob[names(sort(Idents(PC))), names(sort(Idents(PC)))] # 按照subcluster调整行列顺序
...
ggsave(filename = "./ctrl_only/DB_subumap2.png", device = png, height = 4, width = 5, plot = DBsubumap) #对svg调整坐标轴
```
