...
countsMat <- as.matrix(GetAssayData(larva,slot = "counts"))  #data slot counts slot 
...
`FindAllMarkers()`marker，+

`min.pct`marker

```{r markers1, fig.height=8, fig.width=15}
#cluster50123markers,2^0.25
...
#marker We include several tools for visualizing
...
\##`DoHeatmap()` clustermarker

```{r clusterHeatmap, fig.height=8, fig.width=15}
larva.markers %>% group_by(cluster) %>% top_n(n = 6, wt = avg_logFC) -> top10
DoHeatmap(larva, features = top10$gene) + scale_fill_gradientn(colors = c("blue", "white", "red")) #doheatmap
```
...
## Subcluster DB refined_figures.Rmd \## Return: subumap_plot DBsubumap DB_features
...
DB <- RenameIdents(DB,'DB_0'='DB_2') #DB0DB2
...
#gene_name <- c("bnl", "btl", "htl", "Egfr") #"bnl"-FGF , "btl"-FGFR , "htl"-FGFR
 gene_name <- c("grk", "spi", "vn", "Krn") # EgfR  grk, spi, vn, and Krn
...
WhichCells(hsd_larva,idents = 4) #4main10，2main7，1main2，7main1
