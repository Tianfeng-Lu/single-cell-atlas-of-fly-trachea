...
regulonActivity <- regulonActivity[,levels(topTFs$seuratCluster)] #
...
# annotation_row <-  data.frame(cluster = factor(top5$seuratCluster), row.names = make.names(top5$Regulon,TRUE)) #make.names
...
top_anno <- HeatmapAnnotation(cluster=anno_block(gp=gpar(fill=col),labels = cluster_info,
                                                 labels_gp = gpar(cex=1,col='black'))) ## cluster
col_fun <-  colorRamp2(c(-2, 0, 2), c("#1E90FF", "white", "#ff2121")) #
