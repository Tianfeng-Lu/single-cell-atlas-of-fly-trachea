...
# larva <- subset(larva, cells = CellSelector(plot = DimPlot(larva, reduction = "umap"))) #
...
# function set
## nlevels(as.factor(larva$group.by)) 
```{r featureplot}
...
dhm <- function(gene,obj=larva, ...) {DoHeatmap(obj, features = gene, group.colors = colors_list,size = 4) + scale_fill_gradientn(colors = c("#1E90FF", "white", "#ff2121"))} #doheatmap
...
#ggsave(filename = "larva_umap.svg", device = svg, height = 6, width = 7, plot = umap_plot) #svg
```
...
#ggsave(filename = paste0("markers",".tiff"), device = tiff, height = 8, width = 12, plot = markers_heatmap) #svg
```
...
dotplot <- DotPlot(larva, features = top3markers$gene, cols = c("lightgrey","#ff2121")) + coord_flip() + RotatedAxis() + theme_classic() # 
...
#ggsave(filename = paste0(gene_name[1],"vs",gene_name[2],".svg"), device = svg, height = 5, width = 20, plot = doublegene_featureplot) #svg
```
...
# plot <- VlnPlot(larva, features = gene_name, stack=T, pt.size=0, cols = colors_list, # 
#         direction = "vertical", #horizontal   vertical
#         x.lab = '', y.lab = '')+ # 
#   theme(axis.text.y = element_blank(), axis.ticks.y = element_blank()) #

violin_plot <- VlnPlot(larva, features = gene_name, stack=T, pt.size=0, cols = colors_list, # 
...
make_bins <- function(cell.subset, nbins = 11)  
{
  ...
  for (i in c(1:len)) {
    bins[i] <- floor(ordered.cells[i]*nbins/len)  #，bin
  }
...
#ggsave(filename = "larva_subumap.svg", device = svg, height = 6, width = 7, plot = subumap_plot) #svg
...
#ggsave(filename = "VB_subumap.svg", device = svg, height = 6, width = 7, plot = VBsubumap) #svg
