...
## umapplot
```{r}
ctrl$celltype <- factor(ctrl$celltype, levels=c(levels(ctrl_filtered$celltype), setdiff(levels(ctrl$celltype), levels(ctrl_filtered$celltype)))) #         
...
# ctrl.markers <- read.csv("./data_tables/ctrl_markers_fig1.csv", row.names = 1)
```

### cluster volcano plot
see ./utils/scVolcano.R  
```{r}
source("./utils/scVolcano.R")

DEGs_for_volcano <- FindAllMarkers(ctrl_filtered, test.use = "wilcox", logfc.threshold = 0.5, min.pct = 0.1, min.diff.pct = 0.1, only.pos = T, pseudocount.use = 0) %>% filter(p_val_adj < 0.05) %>% filter(!(cluster %in% c())) 

DEGs_for_volcano$avg_log2FC[DEGs_for_volcano$avg_log2FC > 2] <- 2
# DEGs_for_volcano$avg_log2FC[DEGs_for_volcano$avg_log2FC < -2] <- -2

scVolcano(DEGs_for_volcano, annoH = 0.3, myMarkers=c("vg","neur","Dl","N","Ser","E(spl)m2-BFM","E(spl)m3-HLH","E(spl)m7-HLH","E(spl)mbeta-HLH","Tom","peb","lqf","ct"), topGeneN = 3, seed = 143, cluster_colors=colors_list)
...
for(idx in seq(length(levels(Idents(ctrl_filtered))))){
  ...
  print(upset(fromList(ls), order.by = "freq", keep.order = F,text.scale=2, matrix.color = "black",main.bar.color='black',point.size=4, sets.bar.color = "black",mainbar.y.label='#cell',sets.x.label='marginal size', percentage = TRUE, mainbar.y.max=NULL, nintersects = 12)) #      print
  dev.off()
}
...
print(upset(fromList(ls), order.by = "freq", nsets=6, nintersects = 20, keep.order = F,text.scale=2, matrix.color = "black",main.bar.color='black',point.size=4, sets.bar.color = "black",mainbar.y.label='#cell',sets.x.label='marginal size', percentage = TRUE)) #      print
dev.off()
```
...
Dotplot(c(cytokines,"wgn","grnd"), fly_merge_filtered, scale = F)+coord_flip()
```
...
# HSD vs ctrl in PC
## Wilcox test for selected genes
```{r}
