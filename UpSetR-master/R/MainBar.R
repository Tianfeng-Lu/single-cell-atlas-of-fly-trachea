...
Counter <- function(data, num_sets, start_col, name_of_sets, nintersections, mbar_color, order_mat,
                    aggregate, cut, empty_intersects, decrease, trimmed){
  temp_data <- list()
  Freqs <- data.frame()
  ...
  # if(nintersections < nrow(Freqs)){
  #   warning(paste0("Trimmed #intersections exaggerate percentage by: ", sprintf("%1.2f%%", 100*sum(Freqs)/sum(Freqs[1:nintersections, ]))))
  # }
  if(trimmed){
    # if do normalization, then trim those values less than 0.02
    Freqs <- Freqs[round(Freqs$freq/sum(Freqs$freq), 2) >= 0.02, ]
  }
...
Make_main_bar <- function(Main_bar_data, Q, show_num, ratios, customQ, number_angles, number.colors,
                          ebar, ylabel, ymax, scale_intersections, text_scale, attribute_plots, plot.title, total_counts){
  ...
  if(!is.null(total_counts)){
    # if total_counts is not NULL, then do normalization
    Main_bar_data$freq <- round(Main_bar_data$freq/total_counts, 2)
    ymax <- ifelse(is.null(ymax), max(Main_bar_data$freq) + 0.1, ymax) # mainbar.y.max
    # if(scale_intersections != "identity"){
    #   warning("Transformation is not identity for percentage!")
    # }
  }
